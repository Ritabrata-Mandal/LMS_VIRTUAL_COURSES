        ...
        <div className='w-[100%] min-[100vh] flex items-center justify-center flex-wrap gap-[50px] lg:p-[50px] md:p-[30px] p-[10px] mb-[40px]'>
            {
                latestReview?.map((review,index)=>{
                    console.log("jfhfhfgfg",review.user)
                    return (<ReviewCard key={index} comment={review.comment} rating={review.rating} photUrl={review.user.photoUrl} courseTitle={review.course.title} description={review.user.description} name={review.user.name}/>)
                })
            }
        </div>
    </div>
