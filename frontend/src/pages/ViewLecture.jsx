  ...
  return (
    <div className='min-h-screen bg-gray-50 p-6 flex flex-col md:flex-row gap-6'>
        ...
        <div className='w-full md:w-2/3 bg-white rounded-2xl shadow-md p-6 border border-gray-200'>
          ...
          {/* right or bottom */}
          <div className='w-full md:w-1/3 bg-white rounded-2xl shadow-md border border-gray-200 h-fit p-6'>
          <h2 className='text-xl font-bold mb-4 text-gray-800'>All Lectures</h2>
          <div className='flex flex-col gap-3 mb-6'>

            {selectedCourse?.lectures?.length > 0 ? (selectedCourse?.lectures?.map((lecture,idx)=>{
              return (<button key={idx} onClick={()=>setSelectedLecture(lecture)} className={`flex items-center justify-between p-3 rounded-lg border transition text-left  ${selectedLecture._id == lecture._id ? 'bg-gray-200 border-gray-500' : 'hover:bg-gray-50 border-gray-300'}`}>
            ...
            { creatorData && <div className='mt-4 border-t pt-4'>
              ...
              <div className='flex items-center gap-4'><img src={creatorData?.photoUrl ? creatorData?.photoUrl : img} alt="" className='w-14 h-14 rounded-full object-cover'/></div>
