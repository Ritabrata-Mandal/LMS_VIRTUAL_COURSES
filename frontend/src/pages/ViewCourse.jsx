...
const ViewCourse = () => {
  ...
  const numReviews = (selectedCourse?.reviews?.length || 0);
      ...
      <div className='max-w-6xl  mx-auto bg-white shadow-md rounded-xl p-6 space-y-6 relative'>
          ...
          <div className='flex-1 space-y-2 mt-[20px]'>
            ...
            <div className='flex items-start flex-col justify-between'>
              <div  className='text-yellow-500 font-medium flex gap-2'>
                ...
                <span className='text-gray-400'>{numReviews <= 1 ? `(${numReviews} review)` : `(${numReviews} reviews)`}</span>
              </div>

              <div>
                <span className='text-xl font-semibold text-black'>₹{selectedCourse?.price}</span>{" "}
                <span className='line-through text-sm text-gray-400'>₹ {selectedCourse?.price + 299}</span>
              </div>
            ...
            <div className='bg-white w-full md:w-2/5 p-6 rounded-2xl shadow-lg border border-gray-200'>
              ...
              <div className='flex flex-col'>
                {selectedCourse?.lectures.map((lecture,idx)=>{
                  return (<button key={idx} disabled={!lecture.isPreviewFree} onClick={()=>{ if(lecture.isPreviewFree)
                  {
                      console.log(lecture);
                      setSelectedLecture(lecture);
                  }
                  }} className={`flex items-center gap-3 px-4 py-3 rounded-lg border transition-all duration-200 text-left mt-2 ${lecture.isPreviewFree ? "hover:bg-gray-100 cursor-pointer border-gray-300" : "cursor-not-allowed opacity-60 border-gray-200"} ${selectedLecture?.lectureTitle === lecture?.lectureTitle ? "bg-gray-100 border-gray-400" : ""}`}>
             ...
             <div className='bg-white w-full md:w-3/5 p-6 rounded-2xl shadow-lg border border-gray-200'>
              ...
              <div className='aspect-video w-full rounded-lg overflow-hidden mb-4 bg-black flex items-center justify-center'>
                {
                   selectedLecture?.videoUrl ? <video className='w-full h-full object-cover' src={selectedLecture?.videoUrl} controls/> : <span className='text-white text-sm'>Select a previw lecture to watch</span>
                }
              </div>
        ...
        <div className='mt-8 border-t pt-6'>
          ...
          <div className='mb-4'>
            ...
            <button className='bg-black text-white mt-3 px-4 py-2 rounded hover:bg-gray-800' onClick={handleReview} disabled={loading}>{loading ? <ClipLoader size={27} color='white'/> : "Submit Review"}</button>
        ...
        <div className='flex items-center gap-4 pt-4 border-t'>
          ...
          { creatorData?.photoUrl ? <img src="" alt="" className='border-1 border-gray-200 w-16 h-16 rounded-full object-cover' /> : <img src={img} alt="" className='w-16 h-16 rounded-full object-cover'/> }
        ...
        <div className='w-full transition-all duration-300 py-[20px] flex items-start justify-center lg:justify-start flex-wrap gap-6 lg:px-[80px]'>
          {
            creatorCourses?.map((course,idx)=>{
                return (<Card key={idx} thumbnail={course?.thumbnail} id={course?._id} price={course?.price} title={course?.title} category={course?.category}/>)
            })
          }
        </div>
